...
- 📄 **Status + proof debt overview**
  - See [docs/MECHANIZATION_STATUS.md](docs/MECHANIZATION_STATUS.md) for a reviewer-ready mapping from paper claims to Lean files
  - See [docs/PROOF_DEBT.md](docs/PROOF_DEBT.md) for the current `sorry`/axiom inventory
...
| Paper topic / section | Lean artifact(s) |
| --- | --- |
| §3.1 Resource lattice + contexts | `src/RBTT/Res.lean`, definitions reused across `src/RBTT/Core/*.lean` |
| §3.2 Syntax + typing judgment `Γ ⊢[R;b] t : A` | `src/RBTT/Core/STLC.lean` (types, terms, `HasCost`, `HasBound`) |
| §3.3 Operational cost + soundness statements | `src/RBTT/Core/OpCost.lean` (`Step`, `MultiStep`, `cost_soundness_goal`) |
| §4 SetL / modality semantics | `src/RBTT/Semantics/PresheafSet.lean` (shift functor, ε/δ natural transformations) |
| §5 RB-MLTT extension (true dependent types) | `src/RBTT/Core/ExtrinsicMLTT.lean`, examples in `src/RBTT/Examples/DependentTypeExamples.lean`, design docs in `docs/MLTT_STATUS.md` |
| RB-MLTT bound language + fragment roadmap | `docs/RBMLTT_BOUNDS_SPEC.md`, `docs/RBMLTT_FRAGMENT_SPEC.md`, flagship example in `docs/RBMLTT_FLAGSHIP_EXAMPLE.md` |
